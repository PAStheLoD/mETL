...
class PostgresqlDatabase( metl.database.alchemydatabase.AlchemyDatabase ):
    ...
    def afterOpen( self ):
        ...
        naming_cond = [ col.name for col in self.table.c if col.name in list(map( str.lower, self.getColumnNames() )) ]

        if hasattr( self.target, 'getFieldSetPrototypeCopy' ):
            type_cond = [ name \
                for name, field in list(self.target.getFieldSetPrototypeCopy().getFields().items()) \
    ...
    def getListRecord( self, buffer_item ):
        ...
        return ( str( buffer_item.get( column_name ) ) for column_name in self.getColumnNames() )
    ...
    def alternateInsert( self, buffer ):
        ...
        self.cursor.copy_expert(
            "COPY \"%(schema)s\".\"%(table)s\" (%(columns)s) FROM STDIN WITH CSV NULL 'None' QUOTE '`' ESCAPE '`' DELIMITER ','" % {
                ...
                'columns': ', '.join([ '"%s"' % ( c ) for c in self.getColumnNames() ])
            },
