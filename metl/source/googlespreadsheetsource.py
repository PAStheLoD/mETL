    ...
    def getRecordsList( self ):
        ...
        if not self.public_auth:
            ...
            for row_entry in feed.entry:
                try:
                    entries = row_entry.content.text.decode('utf-8').split(', ')
                    field_names  = [ e.split(': ')[0].strip() for e in entries ]
                    field_values = [ e.split(': ')[1].strip() for e in entries ]
                    content = dict( list(zip( field_names, field_values )) )
