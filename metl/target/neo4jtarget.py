...
class Neo4jTarget( metl.target.base.Target ):
    ...
    def dictToString( self, d, remove = None ):
        ...
        json_str = self.regexp.sub( '`\\1`:', json_str )
        return json_str
    ...
    def getCreateRelationCommand( self, b, key ):
        ...
        return 'MATCH (a%(label_l)s), (b%(label_r)s) WHERE a.`%(key_l)s` = "%(val_l)s" AND b.`%(key_l)s` = "%(val_r)s" CREATE (a)-[:`%(label)s` %(data)s]->(b)' % {
            'label_l': ':`%s`' % ( self.labelLeft ) if self.labelLeft is not None else '',
            'label_r': ':`%s`' % ( self.labelRight ) if self.labelRight is not None else '',
            'label': self.label,
