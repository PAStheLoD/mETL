...
import traceback, sys, metl.field, metl.fieldset, metl.fieldmap, copy, \
        metl.tarr.compiler_base, metl.transform.base, inspect
...
class ConfigParser( object ):
    ...
    def getParameterErrorMessage( self, m, entry, params ):
        ...
        s += 'Given:\n' + '\n'.join( [ ( 4*' ' + '%s: %s' % ( str(k), str(v) ) ) for k,v in list(params.items()) ] )
    ...
    def loadSourceObject( self, cfg, base = False ):
        ...
        for fcfg in cfg.get( 'fields', [] ):
            ...
            if fcfg['name'] not in list(defaultValues.keys()) and 'defaultValue' in field_params:
                defaultValues[ fcfg['name'] ] = field_params['defaultValue']

            if 'map' in fcfg:
                m.setdefault( fcfg['name'], fcfg['map'] )

            if fcfg['name'] not in list(defaultValues.keys()) and m.get( fcfg['name'] ) is None:
                m[ fcfg['name'] ] = fcfg['name']

            if fcfg['name'] in list(defaultValues.keys()):
